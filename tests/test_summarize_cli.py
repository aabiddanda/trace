...
def ts_infer_chr1(tmp_path_factory):
    ...
    exit_status = os.system(f"trace-infer -i 2 --npz-files {out_fp} --chroms chr1 -o {outfix}")
    assert exit_status == 0
...
def ts_infer_chr2(tmp_path_factory):
    ...
    assert exit_status == 0
    exit_status = os.system(f"trace-infer -i 2 --npz-files {out_fp} --chroms chr2 -o {outfix}")
    assert exit_status == 0
    assert Path(out_fp2).is_file()
    return out_fp2
...
def test_simple_summarize(ts_infer_chr1):
    ...
    exit_status = os.system(f"trace-summarize -f {ts_infer_chr1} --chroms chr1 -o {outfix}")
    assert exit_status == 0
    assert Path(out_fp).is_file()
    # TODO: some other small assertions of the file
...
def test_multichrom_summarize(ts_infer_chr1, ts_infer_chr2):
    ...
    out_fp = Path(ts_infer_chr1).with_suffix(".summary.txt")
    outfix = Path(ts_infer_chr1).with_suffix("")
    exit_status = os.system(f"trace-summarize -f {ts_infer_chr1},{ts_infer_chr2} --chroms chr1,chr2 -o {outfix}")
    assert exit_status == 0
    assert Path(out_fp).is_file()
    # TODO: some other small assertions of the file
...
def test_bad_ppthresh(ts_infer_chr1, ppthresh):
    """Test extraction of a standard tszip file."""
    outfix = Path(ts_infer_chr1).with_suffix("")
    exit_status = os.system(f"trace-summarize -f {ts_infer_chr1} --posterior-threshold {ppthresh} --chroms chr1 -o {outfix}")
    assert exit_status != 0
...
def test_bad_gendist(ts_infer_chr1, gendist):
    """Test extraction of a standard tszip file."""
    outfix = Path(ts_infer_chr1).with_suffix("")
    exit_status = os.system(f"trace-summarize -f {ts_infer_chr1} --genetic-distance-threshold {gendist} --chroms chr1 -o {outfix}")
    assert exit_status != 0
...
def test_bad_physdist(ts_infer_chr1, bpdist):
    """Test extraction of a standard tszip file."""
    outfix = Path(ts_infer_chr1).with_suffix("")
    exit_status = os.system(f"trace-summarize -f {ts_infer_chr1}  --physical-length-threshold {bpdist} --chroms chr1 -o {outfix}")
    assert exit_status != 0
